...
namespace gazebo
{
  namespace rendering
  {
    // Private implementation data for SelectionBuffer.
    struct SelectionBufferPrivate
    {
      /// \brief This is the material listener. It is controlled by a separate
      /// RenderTargetListener, not applied globally to all targets.
      /// The class associates a color to an Ogre entity.
...
void SelectionBuffer::Update()
{
  ...
  // Try-catch block to prevent crash in deferred rendering mode.
...
void SelectionBuffer::CreateRTTBuffer()
{
  ...
  catch(...)
  {
    ...
    RCLCPP_ERROR(rclcpp::get_logger("SelectionBuffer"),
                 "Unable to create selection buffer.");
    return;
  }
...
/////////////////////////////////////////////////
// OnSelectionClick returns the Ogre entity at the specified pixel.
Ogre::Entity *SelectionBuffer::OnSelectionClick(int _x, int _y)
{
  ...
  // See http://docs.ros.org/indigo/api/rviz/html/c++/selection__manager_8cpp.html
  unsigned int width = 1;
  unsigned int height = 1;
  float x1 = static_cast<float>(_x) / static_cast<float>(targetWidth - 1) - 0.5f;
  float y1 = static_cast<float>(_y) / static_cast<float>(targetHeight - 1) - 0.5f;
  float x2 = static_cast<float>(_x + width) / static_cast<float>(targetWidth - 1) - 0.5f;
  float y2 = static_cast<float>(_y + height) / static_cast<float>(targetHeight - 1) - 0.5f;
  ...
  if (!this->dataPtr->buffer)
  {
    RCLCPP_ERROR(rclcpp::get_logger("SelectionBuffer"), "Selection buffer is null.");
    return nullptr;
  }
  std::memcpy(static_cast<void *>(&color), this->dataPtr->buffer + posInStream, 4);
  ignition::math::Color cvColor;
  cvColor.SetFromARGB(color);
  cvColor.A(1.0);
  const std::string &entName =
    this->dataPtr->materialSwitchListener->GetEntityName(cvColor);
...
/////////////////////////////////////////////////
// Create overlays for debugging the selection buffer.
void SelectionBuffer::CreateRTTOverlays()
{
  ...
  {
    RCLCPP_INFO(rclcpp::get_logger("SelectionBuffer"),
      "Unable to create selection buffer overlay. This will not affect Gazebo unless you're trying to debug the selection buffer.");
  }
}
...
{
  RCLCPP_ERROR(rclcpp::get_logger("SelectionBuffer"),
             "Selection debug overlay disabled for Ogre > 1.9");
}
#endif
