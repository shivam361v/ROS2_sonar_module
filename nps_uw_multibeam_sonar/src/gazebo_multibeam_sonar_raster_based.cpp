...
public:
    ...
    void Load(sensors::SensorPtr _parent, sdf::ElementPtr _sdf) override
    {
        ...
        {
            RCLCPP_ERROR(rclcpp::get_logger("NpsGazeboRosMultibeamSonar"), "DepthCameraPlugin not attached to a depthCamera sensor");
            return;
        }
        ...
        // Make sure the ROS2 node for Gazebo has already been initialized
        if (!rclcpp::ok())
        {
            RCLCPP_FATAL(rclcpp::get_logger("depth_camera"), "A ROS2 node for Gazebo has not been initialized, unable to load plugin.");
            return;
        }
        ...
        {
            RCLCPP_ERROR(rclcpp::get_logger("NpsGazeboRosMultibeamSonar"), "SonarDummy failed to load. Camera and/or Scene not found");
        }
        // load the fiducials
        ...
        {
            RCLCPP_INFO(rclcpp::get_logger("NpsGazeboRosMultibeamSonar"), "No fiducials specified. All models will be tracked.");
            this->detectAll = true;
        }
        ...
        RCLCPP_INFO(rclcpp::get_logger("NpsGazeboRosMultibeamSonar"), "==================================================");
        RCLCPP_INFO(rclcpp::get_logger("NpsGazeboRosMultibeamSonar"), "============   SONAR PLUGIN LOADED   =============");
        RCLCPP_INFO(rclcpp::get_logger("NpsGazeboRosMultibeamSonar"), "==================================================");
        RCLCPP_INFO(rclcpp::get_logger("NpsGazeboRosMultibeamSonar"), "============      RASTER VERSION     =============");
        RCLCPP_INFO(rclcpp::get_logger("NpsGazeboRosMultibeamSonar"), "==================================================");
        RCLCPP_INFO(rclcpp::get_logger("NpsGazeboRosMultibeamSonar"), "Maximum view range  [m] = %f", this->maxDistance);
        RCLCPP_INFO(rclcpp::get_logger("NpsGazeboRosMultibeamSonar"), "Distance resolution [m] = %f", this->soundSpeed*(1.0/(this->nFreq*delta_f)));
        RCLCPP_INFO(rclcpp::get_logger("NpsGazeboRosMultibeamSonar"), "# of Beams = %d", this->nBeams);
        RCLCPP_INFO(rclcpp::get_logger("NpsGazeboRosMultibeamSonar"), "# of Rays / Beam (Elevation, Azimuth) = (%d, %d)", ray_nElevationRays, ray_nAzimuthRays);
        RCLCPP_INFO(rclcpp::get_logger("NpsGazeboRosMultibeamSonar"), "Calculation skips (Elevation) = %d", this->raySkips);
        RCLCPP_INFO(rclcpp::get_logger("NpsGazeboRosMultibeamSonar"), "# of Time data / Beam = %d", this->nFreq);
        if (!this->constMu)
        {
            if (this->customTag)
                RCLCPP_INFO(rclcpp::get_logger("NpsGazeboRosMultibeamSonar"), "Reflectivity method : Variational (based on custom SDF tag)");
            else
                RCLCPP_INFO(rclcpp::get_logger("NpsGazeboRosMultibeamSonar"), "Reflectivity method : Variational (based on model name)");
        }
        else
        {
            RCLCPP_INFO(rclcpp::get_logger("NpsGazeboRosMultibeamSonar"), "Reflectivity method : Constant");
        }
        RCLCPP_INFO(rclcpp::get_logger("NpsGazeboRosMultibeamSonar"), "==================================================");
        RCLCPP_INFO(rclcpp::get_logger("NpsGazeboRosMultibeamSonar"), "");
        ...
        else
        {
            ...
            if (this->writeLogFlag)
            {
                ...
                RCLCPP_INFO(rclcpp::get_logger("NpsGazeboRosMultibeamSonar"), "Raw data at /tmp/SonarRawData_{numbers}.csv");
                RCLCPP_INFO(rclcpp::get_logger("NpsGazeboRosMultibeamSonar"), "every %d frames", this->writeInterval);
                RCLCPP_INFO(rclcpp::get_logger("NpsGazeboRosMultibeamSonar"), "");
    ...
    void PopulateFiducials()
{
    ...
    // Increment and decrement a connection counter so that the sensor
                     ...
                     const std::string &_format)
{
    ...
    // Calculate only if the maxDepth from depth camera is changed and stable
    ...
    if (!this->constMu)
    {
        if (calculateReflectivity)
        {
            ...
            for (const auto &f : this->fiducials)
            {
                ...
                RCLCPP_INFO(rclcpp::get_logger("NpsGazeboRosMultibeamSonar"), "Calculating Reflectivity of captured objects using custom SDF Tags");
                RCLCPP_INFO(rclcpp::get_logger("NpsGazeboRosMultibeamSonar"), "This may take quite some time for the first frame");
                ...
                for (int i = 0; i < reflectivity_image.rows; i++)
                {
                    for (int j = 0; j < reflectivity_image.cols; j += raySkips)
                    {
                        ...
                        if (entity && !entity->getUser ObjectBindings().getUser Any().isEmpty())
                        {
                            try
                            {
                                result = this->scene->GetVisual(
                                    Ogre::any_cast<std::string>(
                                    entity->getUser ObjectBindings().getUser Any()));
                            }
                            catch (Ogre::Exception &_e)
                            {
                                RCLCPP_ERROR(rclcpp::get_logger("NpsGazeboRosMultibeamSonar"), "Ogre Error: %s", _e.getFullDescription().c_str());
                                continue;
                            }
                        }
...
void ComputeSonarImage(const float *_src)
{
    ...
    // If artificial vehicle vibration flag is on
    ...
    if (debugFlag)
    {
        RCLCPP_INFO(rclcpp::get_logger("NpsGazeboRosMultibeamSonar"), "GPU Sonar Frame Calc Time %lld/100 [s]", duration.count() / 10000);
    }
    ...
    if (this->writeLogFlag)
    {
        ...
        if (this->writeCounter == 1 || this->writeCounter % this->writeInterval == 0)
        {
            ...
            filename << "/tmp/SonarRawData_" << std::setw(6) << std::setfill('0') << this->writeNumber << ".csv";
    ...
    // Sonar image ROS msg
    this->sonar_image_raw_msg_.header.frame_id = this->frame_name_.c_str();
    this->sonar_image_raw_msg_.header.stamp.sec = this->depth_sensor_update_time_.sec;
    this->sonar_image_raw_msg_.header.stamp.nsec = this->depth_sensor_update_time_.nsec;
    ...
    this->sonar_image_msg_.header.stamp.nsec = this->depth_sensor_update_time_.nsec;
...
void ComputeCorrector()
{
    ...
    for (size_t beam = 0; beam < nBeams; beam++)
    {
        float beam_azimuthAngle = atan2(static_cast<double>(beam) - 0.5 * static_cast<double>(width), fl);
        for (size_t beam_other = 0; beam_other < nBeams; beam_other++)
        {
            float beam_azimuthAngle_other = atan2(static_cast<double>(beam_other) - 0.5 * static_cast<double>(width), fl);
...
{
    RCLCPP_DEBUG(rclcpp::get_logger("depth_camera"), "publishing default camera info, then depth camera info");
    ...
    void Advertise()
    {
        this->depth_image_pub_ = this->create_publisher<sensor_msgs::msg::Image>(this->depth_image_topic_name_, 10);
        this->depth_image_camera_info_pub_ = this->create_publisher<sensor_msgs::msg::CameraInfo>(this->depth_image_camera_info_topic_name_, 10);
        this->normal_image_pub_ = this->create_publisher<sensor_msgs::msg::Image>(this->depth_image_topic_name_ + "_normals", 10);
        this->point_cloud_pub_ = this->create_publisher<sensor_msgs::msg::PointCloud2>(this->point_cloud_topic_name_, 10);
        this->sonar_image_raw_pub_ = this->create_publisher<marine_acoustic_msgs::msg::ProjectedSonarImage>(this->sonar_image_raw_topic_name_, 10);
        this->sonar_image_pub_ = this->create_publisher<sensor_msgs::msg::Image>(this->sonar_image_topic_name_, 10);
    }
...
int main(int argc, char **argv)
{
    ...
    auto node = std::make_shared<gazebo::NpsGazeboRosMultibeamSonar>();
    rclcpp::spin(node);
    rclcpp::shutdown();
    return 0;
}